 ...
 *
 * Description:
 *	The system layer upon the hiddev interface to DCI HID 16 Photo/Relay device.
 *
 * History:
 *      2009/04/3: initially created
 *              David Kao
 *
 */
#include <stdlib.h>
...
u_int32_t 
dcihid_open(const char *dev_name, const u_int card_id, const u_int card_num)
{
	...
	/*
	 * ioctl() requires a file descriptor, so we check we got one, 
	 * and then open it
	 */
	...
	if ((fd = open(devname, O_RDONLY)) < 0) {
    		...
    		return (u_int32_t)DCIHID_DEV_NULL;
  	}
	...
	for (yalv = 0; yalv < field_info.maxusage; yalv++) {
		...
		case eCard_ID:
			...
			break;
		case eCard_Num:
			if (usage_ref.value != card_num) matched = 0;
			break;
		}
	}
	if (!matched) return (u_int32_t)DCIHID_DEV_NULL;
	...
	if (dcihid_dev == DCIHID_DEV_NULL) return (u_int32_t)DCIHID_DEV_NULL;
	...
	return (u_int32_t)dcihid_dev;
} /* dcihid_open() */

int32_t
dcihid_close(u_int32_t const dcihid_handle)
{
...
int32_t
dcihid_write(const u_int32_t dcihid_handle, const u_int32_t addr, const u_int32_t data)
{
	dcihid_dev_t		  dcihid_dev = (dcihid_dev_t)dcihid_handle;
...
int32_t
dcihid_read(const u_int32_t dcihid_handle, const u_int32_t addr, u_int8_t *data)
{
	dcihid_dev_t		  dcihid_dev = (dcihid_dev_t)dcihid_handle;
	int			  fd = dcihid_dev->fd;
	struct hiddev_report_info report_info;
